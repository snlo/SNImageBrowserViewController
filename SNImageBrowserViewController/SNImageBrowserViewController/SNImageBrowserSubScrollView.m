...
@interface SNImageBrowserSubScrollView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation SNImageBrowserSubScrollView
...
- (void)setComProprotion:(CGFloat)comProprotion
{
    ...
    //改变进度时，就需要通知代理
    if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserSubScrollViewDoingDownDrag:)])
    {
        [self.delegate SNImageBrowserSubScrollViewDoingDownDrag:comProprotion];
    }
}
...
//        [self noticeDelegateBeginOrEndDrag:NO];
    ...
    if (self.moveImage == nil)
    {
        ...
        if (([self.imageNamed hasPrefix:@"http"] || [self.imageNamed hasPrefix:@"HTTP"]) && [self.imageNamed containsString:@"://"]) {
            __weak typeof(self) weakSelf = self;
            [self.moveImage sd_setImageWithURL:[NSURL URLWithString:self.imageNamed] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
...
- (void)saveFrameBeginPan
{
    ...
    CGFloat imageBeginY = (imageHeightBeforeDrag < SCREEN_HEIGHT) ? (SCREEN_HEIGHT - imageHeightBeforeDrag) * 0.5 : 0.0;
...
- (void)endPan
{
    ...
    {
        //通知代理结束拖拽，并退回页面
        if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserSubScrollViewDoDownDrag:view:needBack:imageFrame:)])
        {
            [self.delegate SNImageBrowserSubScrollViewDoDownDrag:NO view:self needBack:YES imageFrame:self.moveImage.frame];
        }
    }
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserSubScrollViewDoSingleTapWithImageFrame:)])
    {
        CGFloat imageW = self.mainImageView.yy_width;
        CGFloat imageH = self.mainImageView.yy_height;
        //计算图片imageY需要考虑到图片此时的高
        CGFloat imageY = (imageH < SCREEN_HEIGHT) ? (SCREEN_HEIGHT - imageH) * 0.5 : 0.0;
        imageY = imageY - self.mainScrollView.contentOffset.y;
        //centerX需要考虑到offset
        CGFloat imageX = -self.mainScrollView.contentOffset.x;
        [self.delegate SNImageBrowserSubScrollViewDoSingleTapWithImageFrame:CGRectMake(imageX, imageY, imageW, imageH)];
    }
}
...
- (void)noticeDelegateBeginOrEndDrag:(BOOL)isBegin
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserSubScrollViewDoDownDrag:view:needBack:imageFrame:)])
    {
        [self.delegate SNImageBrowserSubScrollViewDoDownDrag:isBegin view:self needBack:NO imageFrame:CGRectZero];
    }
}
...
- (UIImageView *)mainImageView
{
    if (!_mainImageView)
    {
        ...
        if (([self.imageNamed hasPrefix:@"http"] || [self.imageNamed hasPrefix:@"HTTP"]) && [self.imageNamed containsString:@"://"]) {
            ...
            [_mainImageView sd_setImageWithURL:[NSURL URLWithString:self.imageNamed] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                __strong typeof(weakSlef) self = weakSlef;
                ...
                self->_mainImageView.yy_width = imageViewWidth;
                self->_mainImageView.yy_height = imageViewHeight;
