//
//  SNImageBrowserMainScrollView.m
...
@interface SNImageBrowserMainScrollView () <UIScrollViewDelegate, SNImageBrowserSubScrollViewDelegate>
...
@implementation SNImageBrowserMainScrollView
...
- (void)createUI
{
    ...
    {
        SNImageBrowserSubScrollView *subScrollView = [[SNImageBrowserSubScrollView alloc] initWithFrame:CGRectMake(i * (self.yy_width + MARGIN_BETWEEN_IMAGE), 0, self.yy_width, self.yy_height) imageNamed:self.imageNameArray[i]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    //通知代理
    if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserMainScrollViewChangeCurrentIndex:)])
    {
        [self.delegate SNImageBrowserMainScrollViewChangeCurrentIndex:self.currentIndex];
    }
}
...
{
    //继续向下通知代理
    if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserMainScrollViewDoSingleTapWithImageFrame:)])
    {
        [self.delegate SNImageBrowserMainScrollViewDoSingleTapWithImageFrame:imageFrame];
    }
}
...
- (void)SNImageBrowserSubScrollViewDoDownDrag:(BOOL)isBegin view:(SNImageBrowserSubScrollView *)subScrollView needBack:(BOOL)needBack imageFrame:(CGRect)imageFrame
{
    ...
    {
        if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserMainScrollViewNeedBackWithImageFrame:)])
        {
            [self.delegate SNImageBrowserMainScrollViewNeedBackWithImageFrame:imageFrame];
        }
    }
    else
    {
        ...
        {
            if ([subView isKindOfClass:[SNImageBrowserSubScrollView class]])
            {
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(SNImageBrowserMainScrollViewDoingDownDrag:)])
    {
        [self.delegate SNImageBrowserMainScrollViewDoingDownDrag:dragProportion];
    }
}
